...
var $cleanSortUsers       = $('clean-sort-users');
var $cleanSortSubmissions = $('clean-sort-submissions');
//var $getWatchlist = $('get-watchlist');
//var $getFavorites = $('get-favorites');
var $filterName       = $('name');
var $filterID         = $('id');
...
var Editor = {
	...
	getName: function () {
		return $filterName.value || 'untitled';
	},
	...
	update: function (filter, forceChange) {
		if (filter && (Editor.changesSaved || forceChange)) {
			if (typeof filter === 'object') {
				...
				Editor.setType(filter.type || filter.mode || filter.whitelist);
				Editor.setUsers(filter.users || filter.contents);
	...
	updateTagPreview: function () {
		...
		$filterTagPreview.setStyle({backgroundColor: color, borderColor: color});
		if (enabled) {
	...
	randomColor: function () {
		...
		Editor.setColor(Utils.cssColor(color));
		Editor.updateTagPreview();
	...
	load: function (ID) {
		if (!Editor.changesSaved && confirm('Save changes before discarding?')) {
			Editor.save();
		}
	...
	'delete': function () {
		var id = Editor.getID();
		if (id && confirm('Are you sure you want to delete this filter? This action cannot be undone.')) {
...
function getWatchlist() {
	// TODO: find some API to get watchlists
	...
	function getPage(p) {
		...
		.then(function (dom) {
			console.log('Got page ' + p + ' of ' + username + '\'s watchlist');
			var users = $A(dom.querySelectorAll('#userpage-budlist>tbody>tr>td>a,div.watch-row>a')).pluck('textContent');
			if (users.length) {
				wl = wl.concat(users);
				return getPage(p+1);
			} else {
...
function getFavorites() {
	// TODO: find some API to get favorites
	return alert('Sorry, not implemented yet! (I would like to know how, tho...)');
}
...
$randomColor.whenClicked(Editor.randomColor);
$cleanSortUsers.whenClicked(Editor.cleanAndSortUserContents);
$cleanSortSubmissions.whenClicked(Editor.cleanAndSortSubmissionContents);
...
//$getWatchlist.whenClicked(getWatchlist);
//$getFavorites.whenClicked(getFavorites);
