...
var $Window = (function () {
	...
	// insert a button into the webpage nav container, along with an icon
	var $icon = html('img', {src: browser.runtime.getURL('static/fabl-32.png')});
	$icon.setStyle({display: 'inline', width: '16px', height: '16px', marginRight: '6px'});
	var text = i18n.get('pageShowWindowText');
	var $showWindow = html('li', {id:'show-app-window'}, html('a', {href:'#'}, [$icon, text]));
	$showWindow.whenClicked(function (e) {
		$window.goto({x:10,y:50});
		$window.update(Users);
	});
	var $searchBar = document.querySelector('.search-box-container');
	$searchBar.parentElement.insertBefore($showWindow, $searchBar);
	i18n.localizeDocument($showWindow);
	
	// insert the app root and window into the document body
	var $App = html('div', {id: 'app'}, $window);
