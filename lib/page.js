...
var Target = Class.create({
	...
	createTagContainer: function () {
		var target = this;
		var $container = html('div', {id: 'tags'});
	...
	createTableRow: function ($table) {
		...
		var $row = html('tr', {id: this.id}, [
			html('td', {}, $link).addClassName('name-column'),
			html('td', {}, $container).addClassName('tags-column'),
			html('td', {}, $dropdown).addClassName('opts-column')
		]);
				...
				<!-- Row structure is as follows
					<tr id="target.id">
						<td class="name-column">
							<a href="/link/to/target">
						<td class="tags-column">
							<div id="tags">
								<span class="tag" id="filter.id">
									<span class="text">
									<button class="remove">
						<td class="opts-column">
...
var $Window = (function () {
	...
	var $tableUsersTitle  = html('h3', {}, 'Users');
	var $tableSubsTitle   = html('h3', {}, 'Submissions');
	var $tableUsers       = html('table', {id: 'users'});
	var $tableSubmissions = html('table', {id: 'submissions'});
	
	var $usersContainer   = html('div', {}, $tableUsers).addClassName('group').addClassName('fixed-height');
	var $subsContainer    = html('div', {}, $tableSubmissions).addClassName('group').addClassName('fixed-height');
	
	var $header = html('div', {id: 'app-header'}, [$title, $remove]).addClassName('drag-handle');
	var $body   = html('div', {id: 'app-body'},   [$search, $tableUsersTitle, $usersContainer, $tableSubsTitle, $subsContainer]);
	...
	Object.extend($window, {
		...
		update: function (targets) {
			...
			if (!$tableUsers.firstElementChild) {
				$tableUsers.appendChild(html('tr', {}, html('td',{colspan:3},'No users found...')));
			}
			if (!$tableSubmissions.firstElementChild) {
				$tableSubmissions.appendChild(html('tr', {}, html('td',{colspan:3},'No submissions found...')));
			}
		...
		search: function (text) {
			...
			$A($tableUsers.querySelectorAll('tr')).forEach(function ($row) {
			...
			$A($tableSubmissions.querySelectorAll('tr')).forEach(function ($row) {
